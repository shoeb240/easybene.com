...
class ScrapeNaviaController extends Zend_Controller_Action
{
    ...
    public function executeAction()
    {
        ...
        foreach($usersAll as $userId => $userObj) {
            $headerArray = $this->getHeaderArr();
            try {
                $resultNaviaStatements = $this->myExecutionResult($userObj['navia_statements']->exe_id, $headerArray);
                $resultNaviaDayCare = $this->myExecutionResult($userObj['navia_day_care']->exe_id, $headerArray);
                $resultNaviaHealthCare = $this->myExecutionResult($userObj['navia_health_care']->exe_id, $headerArray);
                $resultNaviaHealthSavings = $this->myExecutionResult($userObj['navia_health_savings']->exe_id, $headerArray);
            } catch (Exception $e) {
                //echo $e->getMessage();
                //die('catch');
                $this->ret_res = false;
