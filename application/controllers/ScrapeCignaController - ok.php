...
class ScrapeCignaController extends Zend_Controller_Action
{
    ...
    public function executeAction()
    {
        ...
        foreach($usersAll as $userId => $userObj) {
            $headerArray = $this->getHeaderArr();
            try {
                $resultCignaMedical = $this->myExecutionResult($userObj['cigna_medical']->exe_id, $headerArray);
                $arr['cigna_medical'] = json_decode($resultCignaMedical, true);
                $resultCignaMedicalDeductibleClaim = $this->myExecutionResult($userObj['cigna_deductible_claim']->exe_id, $headerArray);
                $arr['cigna_deductible_claim'] = json_decode($resultCignaMedicalDeductibleClaim, true);
                $resultCignaMedicalDetails = $this->myExecutionResult($userObj['cigna_claim_details']->exe_id, $headerArray);
