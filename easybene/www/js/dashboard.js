    ...
    dyn_functions['cigna_medical_table'] = function (result) {
        if (result.claim_details[0]) {
            $.each(result.claim_details, function(key, row) {
                status = 'Pending';
                cssclass = 'pending';
                $('#medical_claim').append('<tr><td>' + row.claim_processed_on + '</td><td>' + row.for + '</td><td>' + row.service_amount_billed + '</td><td>' + row.service_what_i_owe + '</td><!--<td><span class="' + cssclass + '">' + status.toLowerCase() + '</span></td></td>--></tr>');
            });
        } else {
    ...
    dyn_functions['guardian_dental_table'] = function (result) {
        if (result.claim[0]) {
            $.each(result.claim, function(key, row) {
                ...
                $('#dental_claim').append('<tr><td>' + row.paid_date + '</td><td>' + row.patient_name + '</td><td>' + row.submitted_charges + '</td><td>' + row.i_owe + '</td><td><span class="' + cssclass + '">' + status.toLowerCase() + '</span></td></tr>');
            });
        } else {
